...
class RealTimePublisher {
 ...
 public:
  static std::expected<RealTimePublisher, std::error_code> create(const std::string& topic_id,
    ...
    auto e_topic = get_topic(topic_id, mpmc::message_buffer<message_type>::required_size_bytes(options.max_num_observers, options.max_num_publishers));
    if (!e_topic) {
    ...
    if (!e_buffer) {
      e_buffer =
          mpmc::message_buffer<message_type>::init_at(e_topic.value()->shm().addr(), e_topic.value()->shm().size(),
    ...
    mpmc::message_buffer<message_type>& buffer = e_buffer.value();
 ...
 public:
  ...
  std::error_code publish(T_Args&&... args) {
    ...
    logging::debug("RealTimePublisher<'{}'>::publish: emplaced message #{} (index: {})", _topic->id(), message_id, index);
 ...
 private:
  RealTimePublisher(Topic&& topic, const publisher::Options& options, mpmc::message_buffer<message_type>&& buffer, std::uint_fast32_t publisher_id)
 ...
 private:
  ...
  inline uint_t _m_build_message_identifier(uint_t index, uint_t local_message_id) {
    return (index << std::numeric_limits<numeric::half_size_int<uint_t>>::digits) | (static_cast<numeric::half_size_int<uint_t>::type>(local_message_id));
  }

 private:
  Topic _topic = nullptr;
  mpmc::message_buffer<message_type> _message_buffer;
  mpmc::message_buffer<message_type>::PerPublisherHeader* _pp_header = nullptr;
  ps::publisher::Options _options;
  access_type _prev_published_message;
  std::uint_fast32_t _publisher_id;
};
