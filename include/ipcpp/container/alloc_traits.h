...
struct allocator_traits {
 ...
 private:
  [[nodiscard]] static constexpr allocator_type get_allocator() requires requires {
  ...
  [[nodiscard]] static constexpr allocator_type get_allocator() requires std::is_default_constructible_v<allocator_type> {
    return allocator_type();
  }

 public:
  [[nodiscard]] static constexpr difference_type allocate(size_type n) {
    if (requires(allocator_type a, size_type s) {
          { a.allocate(s) } -> std::convertible_to<difference_type>;
        }) {
      return {get_allocator().allocate(n), n / sizeof(value_type)};
    } else if (requires(allocator_type a, size_type s, const void* p) {
                 { a.allocate(s) } -> std::convertible_to<pointer>;
                 { a.pointer_to_offset(p) } -> std::convertible_to<difference_type>;
               }) {
      return {get_allocator().pointer_to_offset(get_allocator().allocate(n)), n / sizeof(value_type)};
    } else {
  ...
  [[nodiscard]] static constexpr std::pair<difference_type, size_type> allocate_at_least(size_type n) {
    ...
    } else if (requires(allocator_type a, size_type s) {
  ...
  static constexpr void deallocate_offset(difference_type o, size_type n) {
    deallocate(offset_to_pointer(o), n);
  }

  static constexpr pointer offset_to_pointer(difference_type offset) {
    if constexpr (requires(allocator_type a, difference_type d) {{a.offset_to_pointer(d)} -> std::convertible_to<pointer>;}) {
      return get_allocator().offset_to_pointer(offset);
    } else {
      return static_cast<pointer>(offset);
    }
  }

  static constexpr difference_type pointer_to_offset(pointer p) {
    if constexpr (requires(allocator_type a, pointer ptr) {{a.pointer_to_offset(ptr)} -> std::convertible_to<difference_type>;}) {
      return get_allocator().pointer_to_offset(p);
    } else {
      return static_cast<difference_type>(p);
    }
  }

  template <typename T_p, typename... T_Args>
    requires std::is_constructible_v<T_p, T_Args...>
  static constexpr void construct(T_p* p,
                                  T_Args&&... args) noexcept(std::is_nothrow_constructible_v<T_p, T_Args...>) {
  ...
  static constexpr auto max_size() noexcept { return get_allocator().max_size(); }
};
